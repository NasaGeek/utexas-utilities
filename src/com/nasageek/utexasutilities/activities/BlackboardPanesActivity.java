...
/*	@Override
	...
	private class BlackboardPaneSizer implements PaneSizer {
		...
		//type is the ID to the integer defining the percentage of the width of the screen that a pane should occupy
		@Override
	...
	/*		if (type == BLACKBOARD_PAGER_PANE_TYPE && index == 0)
				return (int) (((double)res.getInteger(R.integer.blackboard_pager_width_percentage))/100 * parentWidth);
			else if (type == BLACKBOARD_EXTERNAL_ITEM_PANE_TYPE)
				return (int) (((double)res.getInteger(R.integer.blackboard_external_item_width_percentage))/100 * parentWidth);
			else if (type == BLACKBOARD_COURSE_MAP_PANE_TYPE)
				return (int) (((double)res.getInteger(R.integer.blackboard_other_width_percentage))/100 * parentWidth);
			else if (type == BLACKBOARD_)
